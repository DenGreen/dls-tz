...
const Search = ({ setSearchData, data }) => {
  const [searchActive, setSearchActive] = useState(false);
  const [searchStr, setSearchStr] = useState("");

  const dataSearch = (e) => {
    const response = data.filter((value) => {
      return value.title.toLowerCase().indexOf(searchStr.toLowerCase()) > -1 ||
        value.text.toLowerCase().indexOf(searchStr.toLowerCase()) > -1
        ? true
        : false;
    });
  ...
  return (
    <div className={style.search}>
      <div className={style.box}>
        <input
          value={searchStr}
          onKeyDown={dataSearch}
          onChange={(e) => setSearchStr(e.target.value)}
          className={
            style.searchInput +
            " " +
            (searchActive ? style.searchInputActive : "")
          }
          type="text"
          name=""
          id=""
